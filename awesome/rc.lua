...
local autosh = string.format("%s/.dotfiles/autostart.sh", os.getenv("HOME") )
awful.spawn(autosh)
...
local passman			 = "keepassxc"
local files				 = "nemo"
local music				 = "spotify"
local scrlocker		 = "cinnamon-screensaver-command -l"
local pctl				 = "%any,chromium"
...
local theme_paths = {
	...
	string.format("%s/.dotfiles/awesome/theme-personal.lua", os.getenv("HOME")),
}

for k,v in pairs(theme_paths) do
	if gears.filesystem.file_readable(v) then
		beautiful.init(v)
	end
end
...
globalkeys = mytable.join(
		...
		awful.key({}, "XF86AudioNext",
				function ()
						os.execute(string.format("playerctl --player=%s next", pctl))
				end,
				{description = "next track", group = "hotkeys"}),
		awful.key({}, "XF86AudioPrev",
				function ()
						os.execute(string.format("playerctl --player=%s previous", pctl))
				end,
				{description = "previous track", group = "hotkeys"}),
		awful.key({}, "XF86AudioPlay",
				function ()
						os.execute(string.format("playerctl --player=%s play-pause", pctl))
				end,
				{description = "play or pause music", group = "hotkeys"}),
		...
		awful.key({ modkey }, "p", function () awful.spawn(passman) end,
							{description = "run password manager", group = "launcher"}),
		awful.key({ modkey }, "n", function () awful.spawn(files) end,
